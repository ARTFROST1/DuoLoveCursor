...
router.get("/:userId", async (req, res) => {
    try {
        ...
        sessions.forEach((s) => {
            if (s.resultJson && s.resultJson.reactionMs) {
                reactionValues.push(s.resultJson.reactionMs);
            }
        });
        ...
        if (partnership) {
            ...
            // Дни вместе
            const daysTogether = Math.max(1, Math.floor((Date.now() - new Date(partnership.connectedAt).getTime()) / (1000 * 60 * 60 * 24)));
            ...
            const discussionCount = coupleSessions.filter((s) => ["Romantic", "Discussion"].includes(s.game.category)).length;
            coupleStats = {
                daysTogether,
