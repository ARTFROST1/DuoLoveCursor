 ...
 */
export async function processGameSessionResult(opts) {
    const { partner1Id, partner2Id, winnerId } = opts;
    // Determine partnershipId (some old sessions may lack it)
    let resolvedPartnershipId = opts.partnershipId;
    ...
    const partnershipId = resolvedPartnershipId;
    const unlocked = [];
    // Helper to increment progress & unlock if reached goal
    async function incrementCouple(slug, increment = 1) {
        ...
        if (reachGoal && !record.achievedAt)
            unlocked.push(slug);
    }
