...
export class QuizLoveController {
    io;
    room;
    sessionId;
    session;
    ...
    async start() {
        ...
        this.state.questions = shuffled.slice(0, this.state.totalRounds).map((q) => ({
            id: q.id,
            text: q.text,
        ...
        for (const profile of profiles) {
            this.state.profiles[profile.userId] = profile.answersJson;
        }
        ...
        if (needsPrefill.length > 0) {
            ...
            const waitingUsers = [this.session.partner1Id, this.session.partner2Id].filter(id => !needsPrefill.includes(id));
    ...
    async handlePrefill(payload, userId) {
        ...
        const allHaveProfiles = [this.session.partner1Id, this.session.partner2Id].every(id => this.state.profiles[id]);
        if (allHaveProfiles) {
            // Start the game
    ...
    async handleAnswer(payload, userId) {
    ...
    async processRoundResults() {
        ...
        // Update scores
        if (partner1Match)
            this.state.scores[partner1Id]++;
        if (partner2Match)
            this.state.scores[partner2Id]++;
